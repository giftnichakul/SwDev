...
exports.getAppointments=async (req, res, next)=>{
    ...
    try{
        ...
        return res.status(500).json({success:false, msg: "Cannot find Appointments"});
    }
}
...
exports.getAppointment= async (req, res, next)=>{
    try{
        ...
        if(!appointment) return res.status(404).json({success: false, msg:`No appointment with the is of ${req.params.id}`});
...
exports.addAppointment=async(req, res, next) => {
    try{
        ...
        console.log(existedAppointment);
        ...
        if(!hospital) return res.status(404).json({success: false, msg: `No hospital with the id of ${req.params.hospitalId}`});
        const appointment = await Appointment.create(req.body);
        res.status(200).json({success: true, data: appointment});
    }catch(err){
        console.log(err.stack);
        return res.status(500).json({succes: false, msg: "Cannot create Appointment"});
    }
}
...
exports.updateAppointment= async(req, res, next)=>{
    try{
        ...
        if(!appointment) return res.status(404).json({success: false, msg:`No Appointment with the id of ${req.params.id}`});
